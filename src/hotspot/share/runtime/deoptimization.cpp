...
JRT_ENTRY(void, Deoptimization::uncommon_trap_inner(JavaThread* current, jint trap_request)) {
...

  {
    ...
    fprintf(stderr, "This is unloaded_class_index %d, 0x%X. This is bool: %d\n", unloaded_class_index, unloaded_class_index, unloaded_class_index >= 0);
    if (unloaded_class_index < 0) {
      unloaded_class_index = 0;
    }
    fprintf(stderr, "2 This is unloaded_class_index %d, 0x%X. This is bool: %d\n", unloaded_class_index, unloaded_class_index, unloaded_class_index >= 0);
    if (unloaded_class_index >= 0) {
      constantPoolHandle constants(current, trap_method->constants());
